...
build(){
    ...
    cmake -DANDROID_ABI=${ANDROID_ABI} -DCMAKE_VERBOSE_MAKEFILE:BOOL=ON -H. -DCMAKE_TOOLCHAIN_FILE=${ANDROID_NDK}/build/cmake/android.toolchain.cmake -DANDROID_NATIVE_API_LEVEL=${ANDROID_SDK_API_LEVEL} -DANDROID_TOOLCHAIN=clang -DCMAKE_INSTALL_PREFIX=${INSTALL_ROOT_DIR} -S ${CLEARSCRIPT_V8_SRC_DIR} -B ${CLEARSCRIPT_V8_BUILD_DIR}
